...
<div class="position-absolute top-50 start-50 translate-middle">
    <div class="card" style="width: 80vw;height: 90vh">
        <div class="card-body">
          ...
          @endif
          <form action="{{ route("post.add.note") }}" method="POST">
            @csrf

            <div class="form-floating mb-2">
                <input type="text" name="title" maxlength="255" value="@if ($request->session()->exists('title')){{ session('title') }}@endif" class="form-control" id="floatingInputValue" placeholder="Başlık">
                <label for="floatingInputValue">*Başlık (255)</label>
            </div>
            <div class="form-floating">
                <textarea class="form-control" name="note" style="max-height: 70%;height: 74vh;resize:none;" placeholder="Notunuz" id="floatingTextarea2">@if ($request->session()->exists('note')){{ session('note') }}@endif</textarea>
